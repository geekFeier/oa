<template>
	<view class="detailBox">
		<u-navbar :is-back="true" title="留言记录" :border-bottom="false" back-icon-color="#000" :background="background"
	...
	export default {
		...
		methods: {
			...
			changeKeyboardHeight(e) {
				...
				console.log(this.systemInfo.windowHeight,this.bottomActionHeight, e.detail.height,this.chatListHeight)
				this.scrollBottom();
			},
			scrollBottom() {
				setTimeout(() => {
			...
			getData() {
				...
				this.$http("enterprise.message/record", params, "get").then(res => {
					...
					data.forEach(item => {
						...
						if (item.own == 1) {
							item.type = 'right'
							item.avatar = item.userInfo.userinfo.avatar
						} else {
							item.type = 'left'
							item.avatar = item.toUserInfo.userinfo.avatar
						}
					})
