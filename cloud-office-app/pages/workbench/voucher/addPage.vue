<template>
  <view>
    <u-navbar :is-back="true" :title="flag==1 ? '新增凭证'  : '编辑凭证'" :border-bottom="false" back-icon-color="#000" :background="background" title-color="#000" :height="55">
    </u-navbar>
    ...
    <view class="mainBox">
      ...
      <u-form ref="uForm">
        <view class="" v-for="(item,index) in credentials" :key="index">
          <u-form-item label="摘要" label-width="150" :border-bottom="false">
            ...
            <view class="form-item-right" slot="right">
              ...
              <image src="../../../static/image/tab1/caiwu/menuicon.png" style="width: 25rpx;height: 20rpx;" mode="" @click="selectDigest(index)"></image>

            </view>
          </u-form-item>
          <u-form-item label="科目" label-width="150" :border-bottom="false" right-icon="arrow-right" :right-icon-style="{color:'#7d7f97'}">
            <u-input disabled v-model="item.subject_headings" @click="goKeMu(index)" type="input" placeholder="请选择科目" />
          </u-form-item>
          <u-form-item v-for="(_item,indexs) in item.hesuan_children" :key="indexs" :label="_item.name" label-width="150" :border-bottom="false" right-icon="arrow-right" :right-icon-style="{color:'#7d7f97'}">
            <u-input disabled @click="goHsPage(_item.hs_id,index)" v-model="_item.memberName" placeholder="" type="input" />
          </u-form-item>
          <u-form-item label="金额" label-width="150" :border-bottom="false">
            ...
            <view class="passDiv-r">
              ...
              <u-switch size="40" v-model="item.balance_status" active-color="#13C75B" inactive-color="#FF253D">
              </u-switch>
            </view>
          </u-form-item>
          <view class="hr-div" />
        ...
        <u-form-item label="附单据" label-width="150">
        ...
        <u-form-item label="" label-width="150" :border-bottom="false">
         <view class="form-item-img-list">
            <image v-for="(item,index) in formData.documents_images" :src="item" class="form-item-img-list-i" mode="" @click="previewBtn"></image>
          </view>
        </u-form-item>
...
export default {
  ...
  onLoad(e) {
    ...
    this.formData.documents_images = e.imgData ? JSON.parse(e.imgData) : [];
  },
  methods: {
    getday() {
    ...
    previewBtn() {
      uni.previewImage({
        urls: this.formData.documents_images
      })
    }
  }
}
