<template>
  <view>
    <u-navbar :is-back="true" title="辅助核算" :border-bottom="false" back-icon-color="#000" :background="background" title-color="#000" :height="55">
    ...
    <u-modal v-model="isShowAlert" :show-cancel-button="true" :show-confirm-button="true" :show-title="true" title="新增辅助核算类别" @confirm="confirmBtn">
      ...
      this.$http("enterprise.Account_books/index", params, "post").then(res => {
        if (res.data.code == 1) {
          ...
          if (this.userInfo.account_books_id) {
            this.accountText = this.zhangtaoListData.find(item => Number(item.id) === this.userInfo.jobs
              .account_books_id);
